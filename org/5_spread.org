...
- This can be reproduced in R with pseudo-random numbers from ~rnorm~.
  #+begin_src R :session *R* :exports both :results output graphics file :file ../img/binom.png
    par(mfrow=c(2,1))
    hist(rnorm(1e+05))
...
4) What do these results mean?
   ...
   this skewedness. 
   #+end_quote
   #+begin_src R :exports both :session :results output graphics file :file 5_quakebox.png
   ...
   #+RESULTS:
   [[file:5_quakebox.png]]
   #+begin_src R :exports both :session :results output graphics file :file 5_quakehist.png
     hist(y, data=quakes)
   #+end_src
